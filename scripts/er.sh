...
LOGS_REMOTE_PATHS=(
  "/users/kbisenug/decentralized-stream-aggregator-evaluation/src/increasing-number-of-clients/without-aggregator/util/without-aggregator-0-client.csv"
  "/users/kbisenug/decentralized-stream-aggregator-evaluation/src/increasing-number-of-clients/without-aggregator/util/log-0.log"
  "/users/kbisenug/decentralized-stream-aggregator-evaluation/src/increasing-number-of-clients/without-aggregator/util/result-0-client.csv"
  "/users/kbisenug/decentralized-stream-aggregator-evaluation/src/increasing-number-of-clients/without-aggregator/util/logs/RSPEngine.log"
  "/users/kbisenug/decentralized-stream-aggregator-evaluation/src/increasing-number-of-clients/without-aggregator/util/logs/CSPARQLWindow.log"
)
...
download_logs() {
  ...
  for remote_path in "${LOGS_REMOTE_PATHS[@]}"; do
    ...
    scp ${SCP_OPTIONS} -J ${BASTION_USER}@${BASTION_HOST} "${CLIENT_USER}@${CLIENT_HOST}:${remote_path}" "${local_path}/${iteration}/" 2>>"${local_path}/${iteration}/download_errors.log" || \
...
download_replayer_log() {
  ...
  scp ${SCP_OPTIONS} -J ${BASTION_USER}@${BASTION_HOST} "${REPLAYER_USER}@${REPLAYER_HOST}:${LOGS_LOCAL_PATH_REPLAYER}" "${local_path}/${iteration}/"
}
